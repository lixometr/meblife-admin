<template>
  <div v-if="!$store.getters.isLoading">
    ...
    <BtnAdd color="primary" @click="$router.push({name: 'ProductModelNew'})">
      Добавить модель
    </BtnAdd>
      ...
      <template #edit="{item}">
        <td>
          <div class="d-flex">
            <CButton
              color="warning"
              @click="$router.push({name: 'ProductModel', params: {id: item._id}})"
            >
...
export default {
  data() {
    return {
      fetchRoute: "productModelsSearch",
      removeRoute: "productModelById",
      fields: [
        {
          key: "name",
          label: "Название",
